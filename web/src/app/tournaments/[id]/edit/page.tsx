...
import { Tournament, Player, Venue } from "../../../../types";
...
export default function EditTournamentPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (active) setFeedback("Turnier nicht gefunden.");
      }
    })();
    return () => {
      active = false;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        // update participants in separate call
        await fetch(`${API_BASE}/tournaments/${tournament.id}/participants`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-4 p-4 sm:p-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Turnier bearbeiten</h1>
      ...
      <section className="cardish p-5 sm:p-6">
        ...
        {!tournament && !feedback && <div className="text-sm text-slate-300">Lädt...</div>}
        {tournament && (
          <form onSubmit={handleSubmit} className="grid gap-3 md:grid-cols-2">
            ...
            <label className="flex flex-col gap-1 text-sm text-slate-300">
              Start
              <input name="start" type="date" defaultValue={tournament.start || ""} className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-white outline-none focus:border-[#e10600]/60" />
            </label>
            <label className="flex flex-col gap-1 text-sm text-slate-300">
              Ende
              <input name="end" type="date" defaultValue={tournament.end || ""} className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-white outline-none focus:border-[#e10600]/60" />
            </label>
            <label className="flex flex-col gap-1 text-sm text-slate-300 md:col-span-2">
              Notizen
              <textarea name="note" defaultValue={tournament.note || ""} className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-white outline-none focus:border-[#e10600]/60" />
            </label>
            ...
            <div className="md:col-span-2 rounded-lg border border-white/10 bg-white/5 p-3 text-sm text-slate-200 space-y-2">
              ...
              <div className="relative">
                ...
                {participantSearch && (
                  <div className="absolute z-10 mt-1 w-full rounded-lg border border-white/10 bg-[#0f0f12] shadow-lg max-h-48 overflow-auto">
                    {players
                      .filter((p) => `${p.firstName} ${p.lastName}`.toLowerCase().includes(participantSearch.toLowerCase()))
